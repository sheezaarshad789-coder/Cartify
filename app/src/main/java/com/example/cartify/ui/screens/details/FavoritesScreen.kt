...
import com.example.cartify.ui.FavoritesState
import com.example.cartify.ui.FavoritesViewModel
            ...
            when (favoritesState) {
                ...
                is FavoritesState.Error -> {
                    Column(
                        ...
                        Text(text = (favoritesState as FavoritesState.Error).message, color = Color.Red)
                        Button(onClick = { viewModel.loadFavorites() }, modifier = Modifier.padding(top = 8.dp)) {
                            Text("Retry")
                        }
                    }
                }
                is FavoritesState.Success -> {
                    val favoriteProducts = (favoritesState as FavoritesState.Success).products
                    if (favoriteProducts.isEmpty()) {
                        Column(
                            ...
                            Text("Your wishlist is empty", fontWeight = FontWeight.Bold, color = Color.Gray)
                            Text("Save items you like for later", color = Color.Gray, fontSize = 14.sp)
